...
- Component: workspace、node editor、view switch、patch preview
...
3. mapping selectionと3 projection
...
| U-13 | selector | 時間と抽象の入力をtime_abstractionへ分類する | Unit | Must |
| U-14 | selector | 利害対象入力をtime_social_reachへ分類する | Unit | Must |
| U-15 | selector | 因果・矛盾入力をrelationへ分類する | Unit | Must |
| U-16 | projection | roadmapでparent hierarchyを保持する | Unit | Must |
| U-17 | projection | spatial座標を決定的に生成する | Unit | Must |
...
| C-01 | workspace | 初期graphをroadmap表示する | Component | Must |
| C-02 | workspace | view切替で同じnode IDを維持する | Component | Must |
...
| P-02 | performance | 100 nodesでviewを切り替える | Performance | Must |
